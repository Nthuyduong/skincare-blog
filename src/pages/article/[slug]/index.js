...
import { getUserComment, saveUserComment, removeUserComment } from "@utils/local-store";
...
const ArticleDetail = ({ blogProps, isCrs, slug }) => {
    ...
    const fetchComments = async (pageNum) => {
        const res = await fetchCommentByBlogIdApi(blog.id, pageNum, sort);
        if (res?.status) {
    ...
    return (
        <>
            <Head>
                ...
                <meta property="og:image" content={BASE_URL + '/storage/mobile/' + blog.featured_img} />
                <meta property="og:url" content="https://radiance-aura.blog/" />
                <meta name="og:type" content="article" />

                <meta name="twitter:card" content="summary" />
                <meta name="twitter:domain" content="radiance-aura.blog" />
                                ...
                                {/* <div className="comment py-7">
                                    <div className="flex">
                                        ...
                                        <div className="ml-auto">
                                            <select
                                                className="cmt-select dark:border dark:border-white"
                                                onChange={(e) => setSort(e.target.value)}
                                            >
                                                <option value="created_at:desc">Newest comments</option>
                                                <option value="created_at:asc">Oldest comments</option>
                                            </select>
                                        </div>
                                    </div>
