...
const Sub_destination = ({ categoryProps, postsProps, isCsr, sub_slug, page }) => {
    ...
    const fetchPost = async (page) => {
        setLoading(true);
        const res = await fetchBlogPostsByCategorySlugApi(sub_slug, page, 10, sort.key);
    ...
    const fetchDataCsr = async () => {
        const res = await Promise.all([
            getCategoryBySlugApi(sub_slug),
            fetchBlogPostsByCategorySlugApi(sub_slug)
...
Sub_destination.getInitialProps = async ({ query }) => {
    ...
    try {
        const res = await Promise.all([
            fetch(`${BASE_URL}/api/categories/slug/${sub_slug}`),
            fetch(`${BASE_URL}/api/blogs/category/slug/${sub_slug}?page=${page ?? 1}`)
        ])
